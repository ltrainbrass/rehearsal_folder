...
def get_folders(
    ...
    """Returns a list of ids for folders linked in the Google Drive file
    with the input id.

    Args:
        service (Resource): object with methods for interacting with the
            Google Drive API
        file_id (str): id of the file to extract linked folder ids from
        table_number (int): the table index (1-indexed) from which to 
            read directory links. 0 if links should not be read from a 
            specific table
    ...
    html = service.files().export_media(
    ...
    for link in soup.find_all('a', href=True):
        ...
        # Extract folder ids from links in HTML exported through Google 
        # Drive API.
        ...
        if folder_id_matches is not None:
            folders.append({'id': folder_id_matches.group(1),
                           'name': str(link.contents[0])})
        else:
            # Folder id could not be identified from link. Link may not 
            # be to a folder.
            logger.debug(
                'No folder id found for \'%s\' - skipping link', link.text)

    return folders


def get_matching_files_in_folder(
        service: Resource,
        folder_id: str,
        keywords: List[str]
    ) -> List[str]:
    """Returns a list of ids for files within the input folder that have 
    names with at least one of the input keywords

    Args:
        service (Resource): object with methods for interacting with the
            Google Drive API
        folder_id (str): a Google Drive folder id
        keywords (List[str]): keywords to search for in names of files 
    ...
    if len(pdf_files) == 0:
        # Folder does not contain any direct file children. Check for 
        # subfolders.
        folders = [file for file in files
                   if file['mimeType'] == 'application/vnd.google-apps.folder']
        if len(folders) != 0:
            # Find latest folder alphabetically, assuming that folder 
            # names are version names.
            last_folder = max(folders, key=lambda folder: folder['name'])
            return get_matching_files_in_folder(
                last_folder['id'], service, keywords)
...
def get_matching_files(
    ...
    Returns:
        List[str]: a list of ids for Google Drive files from the 
            specified folders that have names containing at least one
            keyword
    """
    file_ids = []
    for folder in folders:
        matching_file_ids = get_matching_files_in_folder(
            service, folder['id'], keywords)
        if not matching_file_ids:
            logger.warning(
                'No matching files found for folder with name=\'%s\', id=%s',
...
def create_output_folder(
    ...
    """Creates a new Google Drive folder with the specified name within
    the specified existing Drive folder

    Args:
        service (Resource): object with methods for interacting with the
            Google Drive API
        output_folder_parent (str): the id of the Google Drive folder to
    ...
    for result in results['files']:
        logger.info(
            'Deleting folder with name=\'%s\', id=%s', result['name'],
            result['id'])
...
def copy_files_to_folder(
    ...
    """Creates copies of the specified Google Drive files in the 
    ...
    Args:
        ...
        output_folder_id (str): the id of the Google Drive folder to
            copy files to
        output_folder_name (str): the name of the Google Drive folder to
            copy files to
    """
    i = 0
    for id_group in file_ids:
        for id_name_pair in id_group:
            logger.debug(
                'Copying \'%s\' to the output directory', id_name_pair[1])
            service.files().copy(
        ...
        i += 1
    logger.info('Successfully copied files to the \'%s\' directory',
                output_folder_name)
...
def parse_arguments() -> Namespace:
    ...
    parser.add_argument(
        'config_ini_file',
        help='the file containing the configuration for the app', nargs=1)
    parser.add_argument(
        '--from-table', type=int, default=0,
...
def main() -> None:
    ...
    try:
        ...
        logger.error(
            'Unable to extract required info from config ini file: %s', e)
        sys.exit(1)
    ...
    if len(folders) != 0:
        ...
        output_folder_id = create_output_folder(
            service, output_folder_parent, output_folder_name)
        copy_files_to_folder(
            service, file_ids, output_folder_id, output_folder_name)
